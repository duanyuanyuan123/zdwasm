...
*/
    ...
    int getAailableDataSize()
    {
        ...
        do
        {
            if (decoder == NULL)
            {
                simpleLog("getAailableDataSize, decoder == NULL");
                break;
            }
    ...
    ErrorCode openDecoder(int *paramArray, int paramCount, long videoCallback, long audioCallback, long requestCallback)
    {
        ...
        do
        {
            ...
            if (r != 0)
            {
                ret = kErrorVideoCode_FFmpeg_Error;
                decoder->videoStreamIdx = -1;
            ...
            if (r != 0)
            {
                ret = kErrorAudioCode_FFmpeg_Error;
            ...
            if (decoder->video == 1 && decoder->audio == 1)
            {
                /* video and audio */
                ret = 0;
            }
            ...
            {
                ret = decoder->video == 1 ? 1 : decoder->audio == 1 ? 2
                                                                    : ret;
            }
        ...
        */
            if (decoder->video == 1)
            {
                ...
                params[0] = 1000 * (decoder->avformatContext->duration + 1000) / AV_TIME_BASE;
            ...
            else
            {
                ...
                params[2] = 0;
                params[3] = 0;
                simpleLog("audio only, video params are zero!");
            }
        ...
        // if ((ret != 0 || ret != 1 || ret != 2)  && decoder != NULL)
        // {
        //     av_freep(&decoder);
        // }
        return ret;
    ...
    int sendData(unsigned char *buff, int size)
    {
        ...
        do
        {
            ...
            //simpleLog("sendData -> buff size: %d.", size);
    ...
    ErrorCode decodeOnePacket()
    {
        ...
        do
        {
            if (decoder == NULL)
            {
                ...
                //simpleLog("kErrorCode_Invalid_State");
                break;
            }
            ...
            {
                ret = kErrorCode_Eof;
                simpleLog("kErrorCode_Eof");
                break;
            }

            if (r < 0 || packet.size == 0)
            {
                //simpleLog("r < 0 || packet.size == 0");
                break;
            }
            ...
            do
            {
                ...
                if (ret != kErrorCode_Success)
                {
                    //simpleLog("ret != kErrorCode_Success");
                    break;
                }
