...
int h264Parser::GetAnnexbNALU(NALU_t *nalu)
{
	...
	if (3 != read_buffer(Buf, 3, m_h264_buffer))
	{ //先读3个字节出来，看文件里面是否有足够的字节
		free(Buf);
		Buf = NULL;
		return 0;
	}
 ...
 */
int h264Parser::h264_parser(unsigned char *buffer, const unsigned int bufferlen, HEVCParser::stHDRMetadata &h264info)
{
	...
	m_h264_buffer.endIdx = m_h264_buffer.length;
	m_isSps = 0;
	n = (NALU_t *)calloc(1, sizeof(NALU_t));
	if (n == NULL)
	{
		std::cout << "Alloc NALU Error " << std::endl;
		return 0;
	}

	n->max_size = H264_FRAME_MAX_LEN > bufferlen ? bufferlen : H264_FRAME_MAX_LEN;
	...
	while (m_h264_buffer.length > 0)
	{
		...
		case NALU_TYPE_SPS:
		{
			...
			h264info.type = NALU_TYPE_SPS;
			m_isSps = 1;
			break;
		}
		case NALU_TYPE_PPS:
		{
			h264info.type = NALU_TYPE_PPS;
...
#endif
		if (h264info.type == NALU_TYPE_SLICE)
		{
			h264info.slice_offset = data_offset;
			h264info.slice_length = n->len;
		}
		else if (h264info.type == NALU_TYPE_IDR)
		{
			h264info.idr_offset = data_offset;
			h264info.idr_length = n->len;
		}
		data_offset = data_offset + data_lenth;
		nal_num++;
	}

	h264info.profile_idc = m_h264_paramter.sps[0].profile_idc;
	h264info.level_idc = m_h264_paramter.sps[0].level_idc;
	// std::cout << "m_h264_paramter.sps[0] profile_idc: " << m_h264_paramter.sps[0].profile_idc << std::endl;
	// std::cout << "m_h264_paramter.sps[0].level_idc: " << m_h264_paramter.sps[0].level_idc << std::endl;
...
int h264Parser::h264_parser_Slice_Header(unsigned char *buffer, unsigned int bufferlen)
{
	...
	if (m_h264_paramter.sps[m_h264_paramter.spslen - 1].pic_order_cnt_type == 0)
	{ //第一种POC的求取方法
	...
	else if ((m_h264_paramter.sps[m_h264_paramter.spslen - 1].pic_order_cnt_type == 1) && (!m_h264_paramter.sps[m_h264_paramter.spslen - 1].delta_pic_order_always_zero_flag))
	{ //第二种POC求取方法
...
int h264Parser::h264_parser_sps(unsigned char *buf, unsigned int nLen)
{
	...
	if (nal_unit_type == 7)
	{
		...
		if (m_h264_paramter.sps[m_h264_paramter.spslen].profile_idc == 100 || m_h264_paramter.sps[m_h264_paramter.spslen].profile_idc == 110 || m_h264_paramter.sps[m_h264_paramter.spslen].profile_idc == 122 || m_h264_paramter.sps[m_h264_paramter.spslen].profile_idc == 244 || m_h264_paramter.sps[m_h264_paramter.spslen].profile_idc == 44 || m_h264_paramter.sps[m_h264_paramter.spslen].profile_idc == 83 || m_h264_paramter.sps[m_h264_paramter.spslen].profile_idc == 86 || m_h264_paramter.sps[m_h264_paramter.spslen].profile_idc == 118 || m_h264_paramter.sps[m_h264_paramter.spslen].profile_idc == 128)
		{
...
int h264Parser::h264_sei_message(unsigned char *buf, unsigned int &StartBit)
{
	...
	int last_playload_size_byte = u(8, buf, StartBit);
	; // READ_CODE(8,"last_playload_size_byte");
...
int h264Parser::h264_sei_playload(int payload_type, unsigned char *buf, unsigned int &StartBit)
{
	...
	{ // Mismatched value from HM 8.1
		h264_decode_nal_sei_decoded_picture_hash(buf, StartBit);
		break;
	}
	case H264_SEI_TYPE_MASTERING_DISPLAY_COLOUR_VOLUME:
	{
		h264_mastering_display_colour_volume(buf, StartBit);
		break;
	}
	}
	return payload_type;
